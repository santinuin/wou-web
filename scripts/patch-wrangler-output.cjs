...
// Remove the deploy config generated by the adapter — it conflicts with
// wrangler.json when running wrangler deploy from dist/server/
const deployConfig = path.join(__dirname, '../.wrangler/deploy/config.json');
if (fs.existsSync(deployConfig)) {
  fs.rmSync(deployConfig);
  console.log('postbuild: removed .wrangler/deploy/config.json');
}
